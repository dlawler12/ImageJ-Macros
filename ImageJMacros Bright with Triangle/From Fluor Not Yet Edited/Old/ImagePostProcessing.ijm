...
for (i=start; i<(images+start); i++) {
	...
	if (holder==1){
		...
		run("Combine...", "stack1=[Result of "+subfolder+"-0-0] stack2=AwakeText.tif");
	}
	else{
		open(drive+"SleepText.tif");
		run("Combine...", "stack1=[Result of "+subfolder+"-0-0] stack2=SleepText.tif");
	}
	holder=0;
	saveAs("Tiff", beginning+"\\Plot\\Image_"+(i+1-start)+".tif");
	run("Close All");
