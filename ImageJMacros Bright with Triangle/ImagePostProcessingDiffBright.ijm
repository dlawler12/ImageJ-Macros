...
for (i=start; i<(images+start); i++) {
	...
	run("Make Binary", "method=Triangle background=Light calculate");
	run("Duplicate...", "title="+subfolder+"-1 duplicate range="+deltaT);
	imageCalculator("Difference create stack", subfolder,subfolder+"-1");
